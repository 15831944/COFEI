...
void MotionCtrAttriWidget::createUI()
{
    ...
    m_tableWg->setColumnCount(3);//速度百分率相关内容
    ...
    m_tableWg->setItemDelegateForColumn(2, new ComboBoxDelegate(regnumNames,this));//添加QCombox代理//增加速度百分率列
...
{
    QString axisinfo = ""; //百分比增加"#"
    for(int i = 0; i < 6; ++i)
    {
        if((m_tableWg->item(i,0)->text()!= "")&&(m_tableWg->item(i,1)->text()!= "")&&(m_tableWg->item(i,2)->text()!= ""))
        {
            axisinfo +=m_tableWg->item(i,0)->text() + ":" + m_tableWg->item(i,1)->text() + ";" +  m_tableWg->item(i,2)->text() + "#";
        }
    }
    if(axisinfo != "")
    {

...
VariableInforWidget::VariableInforWidget(QWidget *parent) : QWidget(parent)
{
    ...
    this->setStyleSheet("QPushButton{width:70px;font:14px;border:1px solid white;border-radius:3px;}"
                        ...
                        "QMenu::item:selected {background-color:rgba(255,255,255,100);}");
...
//    DataBaseBatchManager *db2 = new DataBaseBatchManager;
//    if(db2->OpenDb(str+"/cofeidatabasev1.4.db")){
//        bool isok = true;
//        isok = db2->ExcMapTBatchReplaceDB(tablename,DBInstanceManager::getDBInstance()->DBVarStru.regnumMap);
//        db2->ExcMapTBatchReplaceDB(tablename,DBInstanceManager::getDBInstance()->DBVarStru.SwitchMap);
//         db2->ExcMapTBatchReplaceDB(tablename,DBInstanceManager::getDBInstance()->DBVarStru.countMap);
//          db2->ExcMapTBatchReplaceDB(tablename,DBInstanceManager::getDBInstance()->DBVarStru.qpMap);
//    }
...
//    for(auto it = DBInstanceManager::getDBInstance()->DBVarStru.qpMap.begin(); it !=DBInstanceManager::getDBInstance()->DBVarStru.qpMap.end();++it )
//    {
//        QString axisinf0 = QString(it.value().axisidinfo);
//        QStringList list = axisinf0.split(";");
//        QString newinfo = "";
//        qDebug()<<"before:"<<axisinf0;
//        for(int i = 0; i < list.size(); ++i){
//            QString singleaxis = list[i];
//            QStringList axisname = singleaxis.split(":");
//            if(axisname.size() == 2){
//                QString posinfo = axisname[1];
//                QStringList varlist = posinfo.split(",");
//                if(varlist.size() == 1)
//                {
//                    newinfo += singleaxis + "," +"100" + "," +axisname[0] +"_offset" + ";";
//                }
//                 else if(varlist.size() == 2)
//               {
//                 newinfo += singleaxis + "," +axisname[0] +"_offset" + ";";
//                }
//            }
//        }
//        if(newinfo!="")
//        {
//            newinfo.chop(1);
//            strcpy(it.value().axisidinfo,newinfo.toStdString().c_str());
//        }
//         qDebug()<<"keyname"<< it.key()<<"after:"<<QString(it.value().axisidinfo);
//    }
  // m_ModVarqpMap = DBInstanceManager::getDBInstance()->DBVarStru.qpMap;

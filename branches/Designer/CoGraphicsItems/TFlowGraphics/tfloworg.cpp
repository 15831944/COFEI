#include "tfloworg.h"
#include"../FlowPropery/PropertyItems/orgproperty.h"

TFlowORG::TFlowORG(double StartX, double StartY, double StopX, double StopY,
                     const Qt::PenStyle &LineStyle, const int LineWidth,
                     const QColor &LineColor, const QColor &BackColor) :
    TFlowBase(StartX, StartY, StopX, StopY, LineStyle, LineWidth, LineColor, BackColor)
{
    SetType(CoItem::Org);
    PropertyBase = new ORGProperty();
    connectWithText();
}
...
CoItem *TFlowORG::Copy()
{
    TFlowORG *Flow = new TFlowORG();
...
void TFlowORG::Copy(TFlowORG *flow)
{
    if(NULL == flow)
    {
        return;
...
void TFlowORG::Draw(QPainter *painter)
{

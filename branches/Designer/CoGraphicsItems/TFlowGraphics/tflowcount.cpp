...
TFlowCount::TFlowCount(double StartX, double StartY, double StopX, double StopY, const Qt::PenStyle &LineStyle,
     ...
     TFlowBase(StartX, StartY, StopX, StopY, LineStyle, LineWidth, LineColor, BackColor)
{
    ...
    SetType(CoItem::Add);
    PropertyBase = new CountProperty();
    connectWithText();
}

TFlowCount::~TFlowCount()
{

}

CoItem *TFlowCount::Copy()
{

    TFlowCount *flow = new TFlowCount();
...
void TFlowCount::Copy(TFlowCount *Flow)
{
    if(Flow == NULL)
    {
        return;
...
void TFlowCount::Draw(QPainter *painter)
{
